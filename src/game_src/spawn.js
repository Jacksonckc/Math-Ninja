...
export function spawn(difficulty) {
  ...
  const operation = VALS[difficulty].ops[Math.floor(Math.random() * VALS[difficulty].ops.length)]
  ...
  const a = Math.round(Math.random() * VALS[difficulty].maxNumberSize
  ...
  for (let i = 0; i < VALS[difficulty].targetCount; i++) {
    ...
    while (falseAnswer == answer) {
      falseAnswer = Math.round(
        Math.random() * (VALS[difficulty].maxNumberSize *
                          (operation == "+" ? 2 : 1) *
                          (operation == "*" ? VALS[difficulty].maxNumberSize * MULT_REDUCT**2 : 1))
      );
    }
